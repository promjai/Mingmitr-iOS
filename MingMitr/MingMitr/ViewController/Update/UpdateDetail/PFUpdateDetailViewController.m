...
- (void)viewDidLoad
{
    ...
    CGRect frameName = self.titlenews.frame;
    frameName.size = [self.titlenews sizeOfMultiLineLabel];
    [self.titlenews sizeOfMultiLineLabel];
    
    [self.titlenews setFrame:frameName];
    int linesName = self.titlenews.frame.size.height/15;
    self.titlenews.numberOfLines = linesName;
    
    UILabel *descTextName = [[UILabel alloc] initWithFrame:frameName];
    descTextName.textColor = RGB(117, 76, 40);
    descTextName.text = self.titlenews.text;
    descTextName.numberOfLines = linesName;
    [descTextName setFont:[UIFont boldSystemFontOfSize:17]];
    self.titlenews.alpha = 0;
    [self.detailView addSubview:descTextName];
    
    //detail
    self.detailnews.frame = CGRectMake(self.detailnews.frame.origin.x, self.detailnews.frame.origin.y+self.titlenews.frame.size.height-20, self.detailnews.frame.size.width, self.detailnews.frame.size.height);
    ...
    self.detailView.frame = CGRectMake(self.detailView.frame.origin.x, self.detailView.frame.origin.y, self.detailView.frame.size.width, self.detailView.frame.size.height+self.detailnews.frame.size.height+self.titlenews.frame.size.height-30);
    ...
    [self.Api getNewsCommentObjId:[self.obj objectForKey:@"id"] padding:@"NO"];
}
...
- (IBAction)postCommentTapped:(id)sender {
    ...
    if ([self.Api checkLogin] == 0){
        ...
        if (![self.textComment.text isEqualToString:@""]) {
            ...
            [self.Api commentObjId:[self.obj objectForKey:@"id"] content:self.textComment.text];
...
-(void)singleTapping:(UIGestureRecognizer *)recognizer
{
    ...
    [self.delegate PFImageViewController:self viewPicture:self.detailthumb.image];
...
- (void)loadComment {
    if (!noDataDetail){
        ...
        if ([self.internetstatus isEqualToString:@"connect"]) {
            [self.Api getNewsCommentObjId:[self.obj objectForKey:@"id"] padding:self.paging];
        }
    }
}
...
{
    PFUpdateDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PFUpdateDetailCell"];
    if(cell == nil) {
        NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"PFUpdateDetailCell" owner:self options:nil];
